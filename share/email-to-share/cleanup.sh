...
BUCKET=$(aws cloudformation describe-stack-resource --stack-name ${STACK_NAME} --logical-resource-id EmailBucket --query "StackResourceDetail.PhysicalResourceId" --output text --region ${REGION} --profile ${PROFILE})
echo "Clearing out S3 email bucket ${BUCKET}..."
